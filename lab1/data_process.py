...
def data_convert(x, y, m, k):  # 将输入数据进行转换
    x[x <= 40] = 0
    x[x > 40] = 1  # 将 x 中小于等于 40 的元素设为 0，大于 40 的元素设为 1。
    ont_hot_y = np.zeros((m, k))  # 创建一个形状为 (m, k) 的全零矩阵 ont_hot_y
    for t in np.arange(0, m):  # 使用循环，对于每个样本，将 ont_hot_y 中第 t 行的第 y[t] 列设为 1，其余列为 0。
        ont_hot_y[t, y[t]] = 1
    ont_hot_y = ont_hot_y.T  # 将 ont_hot_y 转置
    return x, ont_hot_y
